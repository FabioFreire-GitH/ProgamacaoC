...
void inicializarFila(Fila *f) { //inicialização
    f->inicio = 0;
    f->fim = 0;
}
 
void inicializarPilha(Pilha *p) { //inicialização
    p->topo = -1;
}
 
void inserir(Fila *f, Pessoa p) { 
    if (f->fim < MAX) {
        f->dados[f->fim++] = p; //“Coloque a pessoa na posição atual do fim da fila 
    }                             //e depois avance o fim para a próxima posição.”
}
